...
\title{Lecture 6 --- Review of Concurrency Control }
...
The intended behaviour is that the thread that locked the mutex is the same one to unlock it later on. 
...
We also examined situations where we really did need to enforce writer priority. That required us to build our own complicated solution with semaphores and a mutex.
