...
\begin{enumerate}
	\item \textbf{Binary Interface.} The compiled binary of the kernel does not remain consistent from build to build for a few reasons:
		...
		\end{itemize}
	\item {Source Interfaces.} The API of the kernel can change too. This is a design decision that gives a high degree of freedom to kernel developers to do things:
		\begin{itemize}
...
That post gives a specific example about changing the USB interface from synchronous streams to asynchronous ones, permitting increased throughout of USB devices. 
