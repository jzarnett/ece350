...
\title{Lecture 20 --- Scheduling in Linux }
...
Since version 2.6.23 of the kernel, however, a new scheduling algorithm replaced the $O(1)$ scheduler; it is called the \alert{Completely Fair Scheduler} (CFS).

In turn, it was replaced by EEVDF (Earliest Eligible Virtual Deadline First) -- we'll skip it.
...
A nice value may be in the range -20 to +19 (lower number is still higher priority).
...
The linear ordering of execution time, called \texttt{vruntime} in the earlier diagram, is also called the \alert{virtual run time}.
 ...
 Higher priority processes' \texttt{vruntime} accumulates slower; lower priority processes' \texttt{vruntime} accumulates faster.
...
This could cause all threads of an application to run on a single core instead of all cores. 
