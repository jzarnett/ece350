...
\title{Lecture 10 --- Memory: Segmentation \& Paging }
...
Each of the elements such as the stack, the heap, the standard C library, et cetera, is known as a \alert{segment}.
...
Now that we have multiple pages for each process and they are no longer contiguous, it is insufficient to have a base address and a limit.
...
This might work if the number of entries in the page table is small (like... 8 or 16 entries). 
...
Much earlier, when talking about the basics of computer hardware, we mentioned that memory comes at different levels and different speeds. 
