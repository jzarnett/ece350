...
\title{Lecture 19 --- Scheduling Algorithm Evaluation; UNIX \& Windows}
...
It should provide for efficient use of I/O devices and tends to penalize processor-bound processes to the benefit of I/O bound processes. 
...
More sleep time than run time? The interactivity score is calculated as: $\left(\dfrac{m}{2}\right)\dfrac{r}{s}$

For those that have more run time than sleep time: $m - \left(\dfrac{m}{2}\right)\dfrac{s}{r}$

Equal? It is $\dfrac{m}{2}$. A \textit{lower} score means more interactive.
